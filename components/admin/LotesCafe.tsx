...
export default function LotesCafe() {
  ...
  const { data: todosLotes } = useRead('lote_cafe', 'variedad',        'variedad')

  const variedades = Array.from(
    new Set((todosLotes as any[]).map((l: any) => l.variedad).filter(Boolean))
  ...
  return (
    <CrudPage
      ...
      fields={[
        { key: 'variedad',      label: 'Variedad', type: 'text', required: true, placeholder: 'Ej: Caturra, Castillo, Geisha', description: 'Especie o variedad botánica del café.' },
        { key: 'idfinca',       label: 'Finca de origen', type: 'select', required: true, options: fincas.map(f => ({ value: f.idfinca, label: f.nombre })) },
        { section: 'Datos de cosecha' } as any,
        { key: 'fecha_cosecha', label: 'Fecha de cosecha',    type: 'date',   required: true },
        { key: 'peso_kg',       label: 'Peso / Stock (kg)',   type: 'number', required: true, step: '0.01', min: '0', placeholder: 'Ej: 2500' },
      ...
      filterSelects={[
        ...
        { key: 'idfinca',   label: 'Finca',    options: fincas.map(f => ({ value: String(f.idfinca), label: f.nombre })) },
