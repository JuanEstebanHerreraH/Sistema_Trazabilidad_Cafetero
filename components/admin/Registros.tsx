...
export default function Registros() {
  ...
  const fields = [
    ...
    { key: 'fecha_fin',    label: 'Fecha de finalización',  type: 'datetime-local', required: false },
    {
    ...
    { key: 'notas', label: 'Notas del proceso', type: 'textarea', required: false, colSpan: 'full', placeholder: 'Temperatura, pH, condiciones climáticas, incidencias…' },
  ]
  ...
  return (
    <CrudPage
      title="Registros de Proceso" subtitle="Historial de beneficios aplicados a cada lote" icon="📋"
      table="registro_proceso" idField="idregistro_proceso"
      selectQuery="*, lote_cafe(variedad), proceso(nombre), usuario(nombre)"
      orderBy="fecha_inicio"
      searchKeys={['notas', 'lote_cafe.variedad', 'proceso.nombre', 'usuario.nombre']}
      searchPlaceholder="Buscar por lote, proceso, responsable o notas…"
      columns={[
      ...
      filterSelects={procesos.length > 0 ? [
        ...
        ...(usuarios.length > 0 ? [{ key: 'idusuario', label: 'Responsable', options: usuarios.map(u => ({ value: String(u.idusuario), label: u.nombre })) }] : []),
      ] : undefined}
      dateFilters={[
