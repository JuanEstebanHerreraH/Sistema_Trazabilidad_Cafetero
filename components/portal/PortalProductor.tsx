...
interface Finca {
  ...
  productor: { nombre: string; region: string | null } | null
}
...
interface Lote {
  ...
  estado: string
  idfinca: number
  registro_proceso: { idregistro_proceso: number; proceso: { nombre: string } | null; fecha_inicio: string; fecha_fin: string }[]
}
...
export default function PortalProductor({ usuario }: { usuario: UsuarioPortal }) {
  ...
  const [tab, setTab]         = useState<'fincas' | 'lotes' | 'actividad'>('fincas')
  ...
  const cargar = async () => {
    ...
    const [{ data: f }, { data: l }] = await Promise.all([
      supabase
        .from('finca')
        .select('idfinca, nombre, ubicacion, area_hectareas, productor(nombre, region)')
        .order('nombre'),
      supabase
        .from('lote_cafe')
        .select(`
          idlote_cafe, variedad, fecha_cosecha, peso_kg, estado, idfinca,
          registro_proceso(idregistro_proceso, fecha_inicio, fecha_fin, proceso(nombre))
        `)
  ...
  // Actividad reciente: registros de proceso en todos los lotes
  const actividadReciente = lotes
    .flatMap(l => l.registro_proceso.map(r => ({ ...r, lote: l })))
    ...
    <div>
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-dim)', fontSize: '0.88rem' }}>Administra tus fincas y el estado de tus lotes.</p>
        </div>
        {tab === 'lotes' && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
        {[
          { icon: '🏡', label: 'Fincas',        val: fincas.length,                                      color: 'var(--amber)'   },
          ...
          { icon: '💰', label: 'Vendidos',       val: lotes.filter(l => l.estado === 'vendido').length,   color: 'var(--purple)'  },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.75rem' }}>
        {(['fincas', 'lotes', 'actividad'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '0.4rem 1rem', border: 'none', borderRadius: 'var(--r-md)', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', background: tab === t ? 'var(--primary)' : 'var(--bg-hover)', color: tab === t ? 'var(--primary-fg)' : 'var(--text-soft)', transition: 'all var(--t)' }}>
            {t === 'fincas' ? '🏡 Mis fincas' : t === 'lotes' ? '☕ Mis lotes' : '📋 Actividad reciente'}
          </button>
        ))}
      </div>
      ...
      {loading ? <Spinner /> : tab === 'fincas' ? (
        fincas.length === 0 ? <Empty mensaje="No hay fincas registradas." /> : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
            {fincas.map(f => {
              const lotesDeEstafinca = lotes.filter(l => l.idfinca === f.idfinca)
              const disponibles = lotesDeEstafinca.filter(l => l.estado === 'disponible').length
              return (
                <div key={f.idfinca} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--r-xl)', padding: '1.25rem' }}>
                  <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.05rem', color: 'var(--text)', marginBottom: '0.6rem' }}>🏡 {f.nombre}</div>
                  ...
                  {f.productor?.region && <InfoRow label="Región" val={f.productor.region} />}
                  <div style={{ marginTop: '0.75rem', borderTop: '1px solid var(--border-soft)', paddingTop: '0.6rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: 'var(--text-dim)' }}>
                    <span>{lotesDeEstafinca.length} lote{lotesDeEstafinca.length !== 1 ? 's' : ''} total</span>
                    <span style={{ color: 'var(--green)', fontWeight: 600 }}>{disponibles} disponible{disponibles !== 1 ? 's' : ''}</span>
                  </div>
                </div>
              )
            })}
          ...
          {lotesFiltrados.length === 0 ? <Empty mensaje="No hay lotes con ese estado." /> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
              {lotesFiltrados.map(lote => {
                ...
                const procs = lote.registro_proceso?.map(r => r.proceso?.nombre).filter(Boolean)
                return (
                  <div key={lote.idlote_cafe} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--r-lg)', padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <div>
                      ...
                      {procs && procs.length > 0 && (
                        <div style={{ fontSize: '0.74rem', color: 'var(--blue)', marginTop: '0.2rem' }}>
                          🔬 Procesos: {procs.join(', ')}
                        </div>
                      )}
                    </div>
        ...
        /* Actividad reciente — procesos realizados por catadores */
        actividadReciente.length === 0 ? (
          <Empty mensaje="No hay actividad registrada en tus lotes aún." />
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            <p style={{ fontSize: '0.82rem', color: 'var(--text-dim)', marginBottom: '0.25rem' }}>
              Evaluaciones y procesos realizados por catadores sobre tus lotes:
            </p>
            {actividadReciente.map(r => (
              <div key={r.idregistro_proceso} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--r-lg)', padding: '0.9rem 1.15rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text)', marginBottom: '0.2rem' }}>
                    🔬 {r.proceso?.nombre ?? '—'} — {r.lote.variedad}
                  </div>
                ...
                <span style={{ padding: '0.15rem 0.55rem', borderRadius: '99px', fontSize: '0.68rem', fontWeight: 700, background: 'var(--blue-bg)', color: 'var(--blue)' }}>
                  Lote #{r.lote.idlote_cafe}
                </span>
              </div>
            ))}
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '1rem' }} onClick={() => setModalLote(false)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--r-xl)', padding: '1.75rem', width: '100%', maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              <Field label="Variedad *" value={formLote.variedad} onChange={v => setFormLote(p => ({ ...p, variedad: v }))} placeholder="Ej: Caturra, Colombia, Geisha" />
...
function InfoRow({ label, val }: { label: string; val: string }) {
  return (
    <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.82rem', marginBottom: '0.22rem' }}>
